...
use addons\workwx\library\src\Api\Struct\ApprovalData\ApprovalDataList;
use addons\workwx\library\src\Api\Struct\Batch\Batch;
use addons\workwx\library\src\Api\Struct\Batch\BatchJobArgs;
use addons\workwx\library\src\Api\Struct\CheckinData\CheckinDataList;
use addons\workwx\library\src\Api\Struct\CheckinOption\CheckinOption;
use addons\workwx\library\src\Api\Struct\ExternalContact\WelcomeMsg;
use addons\workwx\library\src\Api\Struct\Invoice\BatchGetInvoiceInfoReq;
use addons\workwx\library\src\Api\Struct\Invoice\BatchUpdateInvoiceStatusReq;
use addons\workwx\library\src\Api\Struct\Menu\Menu;
use addons\workwx\library\src\Api\Struct\Message\Message;
use addons\workwx\library\src\Api\Struct\Oauth\UserDetailByUserTicket;
use addons\workwx\library\src\Api\Struct\Oauth\UserInfoByCode;
use addons\workwx\library\src\Api\Struct\Pay\PayWwSptrans2PocketReq;
use addons\workwx\library\src\Api\Struct\Pay\QueryWorkWxRedpackReq;
use addons\workwx\library\src\Api\Struct\Pay\QueryWwSptrans2PocketReq;
use addons\workwx\library\src\Api\Struct\Pay\SendWorkWxRedpackReq;
use addons\workwx\library\src\Api\Struct\Tag\Tag;
use addons\workwx\library\src\Api\Struct\User\User;
...
use addons\workwx\library\src\Utils\error\SysError;
use addons\workwx\library\src\Utils\Redis;
use addons\workwx\library\src\Utils\Utils;
use addons\workwx\library\src\Api\Struct\Department;
use addons\workwx\library\src\Api\Struct\ExternalContact\Contact;
use addons\workwx\library\src\Api\Struct\ExternalContact\Groupchat;
...
class CorpAPI extends API
{
     ...
     */
    public function __construct($corpId = null, $secret = null, \addons\workwx\library\src\Api\Struct\Redis $redis = null)
    {
    ...
    protected function RefreshAccessToken()
    {
        ...
        if (empty($this->redis)) {
            if (!Cache::get($this->corpId.'_'.$this->secret)) {
                $url = HttpUtils::MakeUrl("/cgi-bin/gettoken?corpid={$this->corpId}&corpsecret={$this->secret}");
                $this->_HttpGetParseToJson($url, false);
                $this->_CheckErrCode();
                Cache::set($this->corpId.'_'.$this->secret, $this->rspJson["access_token"], $this->rspJson['expires_in']);
            }
            $this->accessToken = Cache::get($this->corpId.'_'.$this->secret);
        } else {
            ...
            if (!$redis->exists($this->corpId.'_'.$this->secret)) {
                $url = HttpUtils::MakeUrl("/cgi-bin/gettoken?corpid={$this->corpId}&corpsecret={$this->secret}");
                $this->_HttpGetParseToJson($url, false);
                $this->_CheckErrCode();
                $redis->set($this->corpId.'_'.$this->secret, $this->rspJson["access_token"], $this->rspJson['expires_in']);
            }
            $this->accessToken = $redis->get($this->corpId.'_'.$this->secret);
        }
    ...
    public function UserSimpleList($department_id, $fetchChild)
    {
        ...
        return User::Array2UserSimpleList($this->rspJson);
    }
    ...
    {
        Utils::checkNotEmptyStr($userid, "userid");
        self::_HttpCall(self::EXTERNAL_CONTACT_LIST, 'GET', array('userid' => $userid),false);
        return Contact::Array2ContactList($this->rspJson);
    }

    /**
     * @brief external_userid : 获取客户详情
    ...
    public function ExternalContactGet($external_userid)
    {
        ...
        return Contact::Array2Contact($this->rspJson);
    }

    /**
     * @brief external_userid : 获取客户群列表
     *
     * @link  https://work.weixin.qq.com/api/doc/90000/90135/92113
     *
     * @param string $userid
     ...
     * @throws QyApiError
     */
    public function GroupchatList($offset=0,$limit=100)
    {
        self::_HttpCall(self::EXTERNAL_CONTACT_GROUPCHAT_LIST, 'POST',['offset'=>$offset,'limit'=>$limit]);
        return Groupchat::Array2GroupchatList($this->rspJson);
    }

    /**
     * @brief external_userid : 获取客户群列表详情
     *
     * @link  https://work.weixin.qq.com/api/doc/90000/90135/92113
     *
     ...
     * @throws QyApiError
     */
    public function GroupchatGet($chat_id)
    {
        self::_HttpCall(self::EXTERNAL_CONTACT_GROUPCHAT_GET, 'POST',array('chat_id' => $chat_id));
        return Groupchat::Array2Groupchat($this->rspJson);
    }

    /**
     * @brief msgaudit/groupchat : 获取内部群列表信息
     *
     * @link  https://work.weixin.qq.com/api/doc/90000/90135/92951
     * @return array
     * @throws HttpError
     * @throws InternalError
     * @throws NetWorkError
     ...
     * @throws QyApiError
     */
    public function MsgauditGroupchatList()
    {
        self::_HttpCall(self::MSGAUDIT_GROUPCHAT_LIST, 'GET',[]);
        return $this->rspJson;
    }

    /**
     * @brief msgaudit/groupchat : 获取内部群详情信息
     *
     * @link  https://work.weixin.qq.com/api/doc/90000/90135/92951
     *
     * @param string $roomid
     ...
     * @throws QyApiError
     */
    public function MsgauditGroupchatGet($roomid)
    {
        self::_HttpCall(self::MSGAUDIT_GROUPCHAT_GET, 'POST',array('roomid' => $roomid));
        return $this->rspJson;
    }
